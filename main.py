...
if __name__ == "__main__":
    ...
    ensure_dir("./saved_models/experiment")
    ensure_dir("./results")
    ensure_dir("./results/test")


    model_save_path = f"./saved_models/{config.save_model}/model-{config.ds}-x5_seed{config.seed}.pkl"
    ...
    for k in range(4):
        ...
        if k == 0:
            ...
            train_label = train_dataset.add_set(f"corr_processed_set{1+k}_data_{config.ds}.pkl")
            test_dataset.filename = f"corr_processed_set{2+k}_data_{config.ds}.pkl"
            test_label = test_dataset.load_data()

        print(f'Data Load Success, train_data: {len(train_dataset.label)}; test_data:{len(test_dataset.label)}')
        counter = Counter(train_label)
        loss_weight = round(counter[0] / counter[1], 1)
        print(f"loss weight: 1:{loss_weight}")
        print("Train:", counter, ";Test:", Counter(test_label))
        ...
        for epoch in range(config.epoch):
            ...
            with torch.no_grad():
                ...
                if epoch_f1 > best_f1:
                    ...
                    print(f"本折训练时间：{train_delta // 3600}小时，{(train_delta % 3600) // 60}分钟，{train_delta % 60}秒")
                    acc_epoch.append(best_f1)
                else:
